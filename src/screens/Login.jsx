...
import { Button as LButton, Input as LInput, Icon as LIcon, Checkbox as LCheckbox } from '../designSystem.jsx';

const { useState: lUse } = React;

function LoginScreen({ onLogin, onApply }) {
  const [email, setEmail] = lUse('amara@sahelcc.org');
  const [pw, setPw] = lUse('••••••••••');
  return (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 88px' }}>
        <div style={{ maxWidth: 360, width: '100%', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <LInput label="Email" iconLeft="user" value={email} onChange={e => setEmail(e.target.value)} />
            <LInput label="Password" type="password" value={pw} onChange={e => setPw(e.target.value)} />
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <LCheckbox checked label="Keep me signed in" onChange={() => {}} />
              <a href="#" style={{ fontSize: 'var(--fs-sm)', fontWeight: 600 }}>Forgot password?</a>
            </div>
            <LButton fullWidth onClick={() => onLogin('proponent')} style={{ marginTop: 4 }}>Sign in</LButton>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 20, padding: '12px 14px', borderRadius: 'var(--radius-md)', background: 'var(--green-050)' }}>
            <LIcon name="leaf" size={16} style={{ color: 'var(--brand-primary)', flex: 'none' }} />
          ...
          <div style={{ marginTop: 26, paddingTop: 20, borderTop: '1px solid var(--border-subtle)' }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <LButton variant="secondary" size="sm" iconLeft="dashboard" onClick={() => onLogin('admin')}>Verst admin</LButton>
              <LButton variant="secondary" size="sm" iconLeft="user" onClick={() => onLogin('proponent')}>Proponent</LButton>
            </div>
          </div>
        </div>
      </div>
