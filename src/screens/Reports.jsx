...
import { Select as RpSelect, Button as RpButton, Badge as RpBadge, Alert as RpAlert, Breadcrumb as RpCrumb, Icon as RpIcon, FuelBadge as RpFuel } from '../designSystem.jsx';
import { VC_DATA } from '../data.js';
import { Panel, PageHeader } from '../components/layout.jsx';

const { useState: rpUse } = React;
  ...
  return (
    <div>
      <PageHeader
        ...
        breadcrumb={scope !== 'all' && role === 'admin' ? <RpCrumb items={[{ label: 'All proponents', href: '#' }, { label: D.proponentName(scope) }, { label: 'Reports' }]} /> : null}
        actions={<React.Fragment>
          <RpButton variant="secondary" iconLeft="download">Export CSV</RpButton>
          <RpButton iconLeft="file">Generate PDF</RpButton>
        </React.Fragment>}
      />

      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12, marginBottom: 18, flexWrap: 'wrap' }}>
        <div style={{ width: 220 }}>
          <RpSelect label="Monitoring period" defaultValue="2026-06" options={[{ value: '2026-06', label: '1 – 7 June 2026 (current)' }, { value: '2026-05', label: 'May 2026' }, { value: '2026-q1', label: 'Q1 2026' }]} />
        </div>
        <div style={{ width: 200 }}>
          <RpSelect label="Methodology" defaultValue="gs" options={[{ value: 'gs', label: 'Gold Standard — M&M' }, { value: 'cdm', label: 'CDM AMS-II.G' }]} />
        </div>
        <div style={{ flex: 1 }} />
      ...
      <Panel title="Parameter summary" sub="Per fuel type · current monitoring period" pad={false} style={{ marginBottom: 18 }}
        actions={<RpBadge tone="brand" variant="soft" icon="checkCircle">Verified telemetry</RpBadge>}>
          ...
          <tbody>
            {rows.map(r => (
              <tr key={r.fuel} style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                <td style={{ padding: '11px 16px' }}><RpFuel fuel={r.fuel} size="sm" /></td>
                ...
                <td style={{ padding: '11px 16px', textAlign: 'right' }}><RpBadge size="sm" tone={r.method === 'sensor-direct' ? 'brand' : 'warning'} variant="soft">{r.method === 'sensor-direct' ? 'Sensor-direct' : 'Hybrid'}</RpBadge></td>
      ...
      <RpAlert tone="neutral" icon="file" title="Audit trail">
